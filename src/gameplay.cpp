...
void rotateToPlayer(int i, int distDiff) { // distDiff is how much closer to be than cam angle
    ...
    void addNumber(unsigned char number) {
        string oldIpAddress = currentIpAddressSlot;
        currentIpAddressSlot += number;
    ...
    void backspaceNumber() {
        string oldIpAddress = currentIpAddressSlot;
        if (currentIpAddressSlot.length() > 1) {
            currentIpAddressSlot = currentIpAddressSlot.substr(0, currentIpAddressSlot.length()-1);
        } else {
            currentIpAddressSlot = "0";
        }
        trySettingIpAddress(oldIpAddress);
    }
    ...
    void checkIfFocusChanged(int i) {
        if (focus[i] != oldFocus[i]) {
            resetNumber(focus[i]);
        }
        oldFocus[i] = focus[i];
    }
