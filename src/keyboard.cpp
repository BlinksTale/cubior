...
void handleInput(unsigned char key, bool newBool) {
    ...
    bool skipRealControls = false;
    
    if (typingTime(0) && !justTyped) { // only checks for player 1? FIXME
        checkIfFocusChanged(0); // FIXME: same 0 player bias
        
        justTyped = false;
        // Early switch for typing in IP address
        switch (key) {
            case '0':
            case '1':
            case '2':
    ...
    if (!typingTime(0)) { // only checks for player 1? FIXME
        switch(key) {
                // SYSTEM CONTROLS
    ...
    if (true) {
        switch(key) {
            ...
            case 27 : // Or Esc
                      playerPause(0,newBool,true); break;
